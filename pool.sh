...
_pool_add() {
    ...
    [[ -f $path/$NAME$CFG_EXT ]] && addmsg "Pool '$NAME' FPM definition for PHP$PHPV already exists." $MSG_TYPE_ERR
...
# remove FPM pool
_pool_rm() {
    declare path="$PHP_PATH/$PHPV/fpm/pool.d/$NAME$CFG_EXT"

    [[ -f $path ]] && sudo rm "$path" && addmsg "Pool '$NAME' FPM definition for PHP$PHPV removed."
}
